...
export async function getAvailability(input: { serviceId: string; stylistId: string | null; date: string }) {
  ...
  return { slots: computeOpenSlots({ date: input.date, hours, durationMin: service.duration_min, stepMin: STEP_MIN, busy, tz: TZ }) };
}
...
export async function createBooking(raw: CreateBookingInput): Promise<CreateResult> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!error) {
      ...
      return { ok: true, reference, whenLabel, stylistName, serviceName: service.name, priceLkr: service.price_lkr, durationMin: service.duration_min };
    }
